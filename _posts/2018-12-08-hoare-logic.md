...
我认为学习Hoare Logic能很大缓解这些问题。
无论在刷题练习、白板写题、还是日常工作中，Hoare Logic的思想都是十分受用的。
本文撇去一些严谨的数学定义，着重介绍Hoare Logic的基本规则和思想。

Hoare Logic的核心思路是：任何一支“简单”的程序，都是由三种基本结构相互嵌套组成的：顺序（sequential）、选择（if-then-else）和循环（while-do）。
...
拿上一个例子的Postcondition说，如果x小于10，那么x显然小于100。
...
这个式子的意思是，如果横线上面的三个条件都满足时，那么我们可以得到横线下面的条件。
比如上面的例子就能写成：
...
我们可以通过下面的推到方法得出：程序执行之后满足x小于10，y小于13。
...
我先给出选择结构程序的基本规则**If-Then-Else Rule**，这里为了简化理解，这里我将Hoare的原规则作了不严谨的改写。
...
假如前置条件是x等于-10，那么`P && B`，即`x = -10 && x < 0`为真；`P && !B`，即`x = -10 && x >= 0`为假。
套用第一条规则我们得到：

~~~
{x = -10 && x < 0},  {x = -10} abs := -x; {abs = 10}
---------------------------------------------------------
...
~~~
{sum = 0 + ... + (i - 1) && i <= 11} sum := sum + i; i := i + 1; {sum = 0 + ... + (i - 1) && i <= 11}
~~~

我们套用Sequential Rule可以证明这个的式子。
...
比如动手写代码前如果能提前想好Precondition/Postcondition/Invariant，编写过程往往会事半功倍；
...
面试真正需要考察的，尤其对大公司而言，也不是会多么偏门的数据结构和算法或者特殊领域的知识点，毕竟术业有专攻。
面试者清晰的代码思维和良好的代码习惯才是主要考核项目，毕竟大家大部分代码都在写顺序、选择和循环。
（对了，还有递归，以后有机会再写吧。）